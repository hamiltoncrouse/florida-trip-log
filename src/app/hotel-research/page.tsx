...
type HotelInsights = {
  ...
  positives?: Array<{ rating?: number; text?: string; time?: string; author?: string }>;
  negatives?: Array<{ rating?: number; text?: string; time?: string; author?: string }>;
  url?: string;
};
...
function HotelResearchContent() {
  ...
  const [insights, setInsights] = useState<Record<string, { loading: boolean; error?: string | null; data?: HotelInsights }>>({});
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={async () => {
                      const nextId = expandedHotelId === hotel.id ? null : hotel.id;
                      setExpandedHotelId(nextId);
                      if (nextId && !insights[hotel.id]?.data && !insights[hotel.id]?.loading) {
                        setInsights((prev) => ({ ...prev, [hotel.id]: { loading: true } }));
                        try {
                          const res = await fetch(`/api/maps/hotels/${hotel.id}`);
                          const data = await res.json();
                          if (!res.ok || data.error) throw new Error(data.error || "Failed to research");
                          setInsights((prev) => ({ ...prev, [hotel.id]: { loading: false, data } }));
                        } catch (err) {
                          setInsights((prev) => ({
                            ...prev,
                            [hotel.id]: { loading: false, error: err instanceof Error ? err.message : "Research failed" },
                          }));
                        }
                      }
                    }}
              ...
              {expandedHotelId === hotel.id && (
                <div className="mt-3 rounded-lg border-2 border-dayglo-void bg-dayglo-yellow/20 p-3 shadow-hard-sm">
                  ...
                  <div className="mt-3 space-y-2">
                    {insights[hotel.id]?.loading && <p className="text-xs text-dayglo-void/70">Researching...</p>}
                    {insights[hotel.id]?.error && <p className="text-xs text-rose-500">{insights[hotel.id]?.error}</p>}
                    {insights[hotel.id]?.data && (
                      <div className="space-y-2 rounded-md border border-dayglo-void/40 bg-white/70 p-3">
                        {insights[hotel.id]?.data?.summary && (
                          <p className="text-sm font-semibold text-dayglo-void">{insights[hotel.id]?.data?.summary}</p>
                        )}
                        <div className="grid gap-3 md:grid-cols-2">
                          <div>
                            <p className="text-xs font-black uppercase tracking-[0.2em] text-dayglo-void/70">What people love</p>
                            <p className="text-xs text-dayglo-void/80">
                              {insights[hotel.id]?.data?.positiveSummary || "No positive highlights yet."}
                            </p>
                          </div>
                          <div>
                            <p className="text-xs font-black uppercase tracking-[0.2em] text-dayglo-void/70">Common complaints</p>
                            <p className="text-xs text-dayglo-void/80">
                              {insights[hotel.id]?.data?.negativeSummary || "No recurring complaints noted."}
                            </p>
                          </div>
                        </div>
                        {insights[hotel.id]?.data?.url && (
                          <a
                            href={insights[hotel.id]?.data?.url}
                            target="_blank"
                            rel="noreferrer"
                            className="text-xs font-black uppercase tracking-[0.2em] text-dayglo-pink underline"
                          >
                            Open full Google listing
                          </a>
                        )}
                      </div>
                    )}
                    {!insights[hotel.id] && <p className="text-xs text-dayglo-void/70">Pulling quick highlights…</p>}
                  </div>
                </div>
              )}
            </div>
