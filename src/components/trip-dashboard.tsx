...
const initialChat: ChatMessage[] = [
  {
    id: "intro",
    role: "assistant",
    text: "Hey, I’m Fonda — your neon-loving astral navigator. Ask me for riffs on routes, timing tweaks, or secret detours whenever you need a spark.",
  },
];
...
const getAttachments = (metadata?: Record<string, unknown> | null): Attachment[] => {
  if (!metadata) return [];
  ...
  return raw
    .map((item) => {
      ...
      if (typeof obj.fileName !== "string" || typeof obj.mimeType !== "string" || typeof obj.data !== "string") return null;
      return {
...
export function TripDashboard({
  ...
  const [planDayMode, setPlanDayMode] = useState<"balanced" | "full">("balanced");
...
const sortActivitiesByStart = (activities: Activity[]) =>
  ...
  async function uploadArtifact(activityId: string, file: File) {
    if (!selectedTrip || !selectedDay) return;
    try {
      setUploadingArtifactId(activityId);
      setArtifactError(null);
      const base64 = await fileToBase64(file);
      const res = await fetch(
        `/api/trips/${selectedTrip.id}/days/${selectedDay.id}/activities/${activityId}/artifacts`,
        {
          method: "POST",
          headers: jsonHeaders,
          body: JSON.stringify({
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error(body?.error || `Failed to upload (${res.status})`);
      }
      const data = await res.json();
                    ...
                    ? {
                        ...
                        activities: (day.activities || []).map((activity) =>
                          activity.id === activityId
                            ? {
                                ...activity,
                                metadata: { ...(activity.metadata || {}), attachments: newAttachments },
                              }
                            : activity,
                        ),
                      }
      ...
      setArtifactError(err instanceof Error ? err.message : "Failed to upload file");
    } finally {
  ...
  async function planDayWithFonda() {
    ...
    try {
      ...
      const slots =
        planDayMode === "full"
          ? [
              { label: "Breakfast", start: "08:30" },
              { label: "Morning", start: "10:15" },
              { label: "Early Afternoon", start: "12:30" },
              { label: "Lunch", start: "13:30" },
              { label: "Afternoon", start: "15:30" },
              { label: "Dinner", start: "19:00" },
              { label: "Late Night", start: "21:30" },
            ]
          : [
      ...
      const res = await fetch("/api/ai/suggestions", {
        ...
        body: JSON.stringify({
          ...
          day: selectedDay.date,
          interests: [],
          message: `Plan my day in ${selectedDay.city || "this city"} on ${selectedDay.date}. Return exactly ${
            slots.length
          } items in this exact order: ${slots
            .map((slot) => slot.label)
            .join(
              ", ",
            )}. Each should be at least 90 minutes long. Meals (breakfast, lunch, dinner) should be well-reviewed local spots matching the traveler profile; other slots should be strong activities (walking tours, museums, exhibitions, architecture, seasonal events, shows, night life). Avoid anything already planned in this trip: ${
            existingList || "none"
          }; pick different activities if there's overlap. Avoid ideas that don't fit the season/weather (e.g., winter outdoor swimming). Only return places that are open on that date and at the slot time; if a candidate opens later than the slot, shift the start to the opening time and note it; if closed that day, choose another option. For each suggestion include a street-level address and a short reason to go. Format the description exactly as: "Address: <address> | Why: <why it's worth it>".`,
      ...
      const created: Activity[] = [];
      const count = Math.min(slots.length, uniqueIdeas.length);
      if (!count) {
        throw new Error("No ideas came back. Try again in a moment.");
      }
    ...
    <div className="flex h-full w-full flex-col">
      <div className="flex flex-col gap-2 pb-3">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xl font-black text-dayglo-void leading-tight">Astral Navigator</h2>
        </div>
        </div>
      ...
      <div className="space-y-2 pb-3">
        <div className="flex flex-wrap items-center gap-2">
          <p className="text-[11px] font-black uppercase tracking-[0.15em] text-dayglo-void/70">Plan style</p>
          {[
            ...
            <button
              key={option.key}
              type="button"
              onClick={() => setPlanDayMode(option.key as "balanced" | "full")}
              className={`rounded-md border-2 px-3 py-1 text-[11px] font-black uppercase tracking-[0.2em] transition ${
                planDayMode === option.key
                  ? "border-dayglo-void bg-dayglo-lime text-dayglo-void shadow-hard"
                  : "border-dayglo-void bg-white text-dayglo-void shadow-hard-sm hover:-translate-y-1 hover:shadow-[4px_4px_0px_0px_#FF00FF]"
              }`}
            >
              {option.label}
            </button>
                                ...
                                >
                                  <div className="flex items-center justify-between gap-3">
                                    <div className="space-y-1">
                                      ...
                                      {getAttachments(activity.metadata).length > 0 && (
                                        <div className="space-y-1">
                                          <p className="text-[11px] font-black uppercase tracking-[0.2em] text-dayglo-void/70">
                                            Artifacts
                                          </p>
                                          <div className="flex flex-wrap gap-2">
                                            {getAttachments(activity.metadata).map((attachment) => (
                                              <a
                                                key={attachment.id}
                                                href={`data:${attachment.mimeType};base64,${attachment.data}`}
                                                download={attachment.fileName}
                                                className="rounded-md border border-dayglo-void/40 bg-white px-2 py-1 text-[11px] font-semibold text-dayglo-void shadow-hard-sm hover:bg-dayglo-yellow/50"
                                              >
                                                {attachment.fileName}
                                              </a>
                                            ))}
                                    ...
                                    <div className="flex gap-2 text-[11px] font-black uppercase tracking-[0.2em] text-dayglo-void">
                                      ...
                                      <label className="cursor-pointer rounded-md border-2 border-dayglo-void bg-white px-2 py-1 text-[11px] font-black uppercase tracking-[0.2em] text-dayglo-void shadow-hard-sm transition hover:bg-dayglo-yellow hover:translate-y-[1px] hover:shadow-none">
                                        {uploadingArtifactId === activity.id ? "Uploading…" : "Upload file"}
                                        <input
                                          type="file"
                                          ...
                                          onChange={(e) => {
                                            ...
                                            if (file) uploadArtifact(activity.id, file);
                                            e.target.value = "";
                                          }}
                                          disabled={uploadingArtifactId === activity.id}
                                        />
                                      </label>
                                    </div>
                                    {artifactError && (
                                      <p className="text-xs text-rose-500">Artifact error: {artifactError}</p>
                                    )}
                                  </div>
                                </li>
                              );
